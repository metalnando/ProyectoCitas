...
export class MedicoService {
    ...
    constructor (@InjectModel('Medico') private readonly medicoModel: Model<IMedico>) {}
    ...
    async getMedicoById(id:string): Promise<IMedico>{
        if(!mongoose.Types.ObjectId.isValid(id))
        {
         ...
         if(!medico)
         {
            throw new NotFoundException(`El id ${id} no se enucentra en la base de datos`);
         }
         return medico;
    ...
    async updateMedico(id:string, createMedicoDto:createMedicoDto): Promise<IMedico>{
        if(!mongoose.Types.ObjectId.isValid(id))
        {
            throw new NotFoundException("Id del Medico no es correcto");
        }
        const medicoActualizado = await this.medicoModel.findByIdAndUpdate(id, createMedicoDto, {new:true}).exec();
    ...
    //Eliminar el medico
    async deleteMedico(id:string, createMedicoDto:createMedicoDto): Promise<IMedico>{
        if(!mongoose.Types.ObjectId.isValid(id))
            {
                throw new NotFoundException("Id del Medico no es correcto");
                }
                ...
                if(!medicoEliminado){
                    throw new NotFoundException(`El medico con el ${id} no se encuentra en la base de datos`);
                }
