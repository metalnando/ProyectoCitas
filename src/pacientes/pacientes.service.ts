...
async findById(id: string): Promise<IPaciente> {
  ...
  async actualizarPaciente(id: string, pacienteDto: createPacienteDto): Promise<IPaciente> {

    if(!mongoose.Types.ObjectId.isValid(id)){
      throw new BadRequestException("Id de paciente no valido");
    }

    const pacienteActualizado = await this.pacienteModel.findByIdAndUpdate(id,pacienteDto,{new:true}).exec();
  ...
  // Eliminar Paciente
  async eliminarPaciente(id:string, createPacienteDto:createPacienteDto): Promise<IPaciente>{
    if(!mongoose.Types.ObjectId.isValid(id))
    {
      throw new NotFoundException("El id no es correcto");
    }
    
    const pacienteEliminado = await this.pacienteModel.findByIdAndDelete(id).exec();
